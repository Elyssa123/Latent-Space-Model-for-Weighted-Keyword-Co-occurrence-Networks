...
bingo.N <- data.frame(matrix(nrow=0, ncol=7))
colnames(bingo.N) <- c('N','Ti','D','max.weight','obj','iter','time1')
...
for (u in 1:length(N.set)) {
  ...
  colnames(result0) <- c('N','Ti','D','max.weight','obj','iter','time1')
...
```{r}
d2.1 <- bingo.N
d2.1$N <- as.factor(d2.1$N)

g2.1 <- ggplot(d2.1, aes(x=N, y=time1)) + 
  geom_boxplot() +
  labs(y = 'Time per iteration in seconds', x='Number of nodes')+   #改变坐标名称
...
bingo.T <- data.frame(matrix(nrow=0, ncol=7))
colnames(bingo.T) <- c('N','Ti','D','max.weight',
                     'obj','iter','time1')
...
```{r}
d2.2 <- bingo.T
d2.2$Ti <- as.factor(d2.2$Ti)

g2.2 <- ggplot(d2.2, aes(x=Ti, y=time1)) + 
  geom_boxplot() +
  labs(y = 'Time per iteration in seconds', x='Number of periods')+   #改变坐标名称
...
ggarrange(g2.1, g2.2, labels = c("(a)","(b)"),widths = c(2,2), ncol =2, nrow = 1)
ggsave('simulation-results2.pdf', width = 20, height = 8)
```
