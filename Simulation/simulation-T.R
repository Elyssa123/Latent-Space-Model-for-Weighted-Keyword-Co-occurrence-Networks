...
#----------------------------initialization-------------------------------------
init.func4 <- function(N, D, Ti, node.exist, seed){
  set.seed(seed)
  alpha <- runif(N, -2, -1)
  beta <- runif(Ti, -1, 1)
  Z <- list()
...
main.func <- function(Z.true, alpha.true, beta.true, node.exist, nu, eta, delta, step.size=0.2, epsi=0.00001, niter=500, trace=0, seed=10){
  ...
  para.ini <- init.func4(N, D, Ti, node.exist, seed=seed)
  Z.ini <- para.ini$Z
...
# main.func(Z.true, alpha.true, beta.true, gamma.true, step.size=0.2, epsi=0.0005, niter=500, trace=0)
...
bingo <- data.frame(matrix(nrow=0, ncol=17))
colnames(bingo) <- c('N','Ti','D','max.weight',
                     'Theta.relative.error','Theta.error',
                     'obj','iter','time1')

for (u in 1:length(Ti.set)) {
  set.seed(u^5) 
  Ti <- Ti.set[u]
  print(Ti)
  rho <- c(sort(runif(Ti-1, 0.6, 1)),1) 
  # nodes at each time
  ...
  for (t in 1:Ti) {
    ...
    # Z.true0 <- Z.true0/norm(Z.true0,'F')
  ...
  bingo <- rbind(bingo, result0)
  
}

bingo

time.t <- bingo %>% group_by(Ti) %>% summarise(median(time1))
